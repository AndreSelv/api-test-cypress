Feature: Inland Marine Guide (Non Filed) - IMG

  Scenario: Delete report folder before all tests
    Given Delete "./reports/IMG" folder

  Scenario Outline: The user validates the search result for Product line - Inland Marine Guide (Non Filed), state - Oregon, Publication Category - '<pubCategory>' and Publication Type - '<pubType>' with Data base connectivity
    Then The user call search endpoint with '<pubCategory>' and '<pubType>' and 'IMG' and 'OR' and '01/01/2022' and should get result match with legacy DB search result
    Examples:
      | pubCategory                  | pubType                                         |
