Feature: Personal & Premises Liability - PPL

#  Scenario: Delete report folder before all tests
#    Given Delete "./reports/PIM" folder

  Scenario Outline: The user validates the search result for Product line - Personal & Premises Liability, state - New Hampshire, Publication Category - '<pubCategory>' and Publication Type - '<pubType>' with Data base connectivity
    Then The user call search endpoint with '<pubCategory>' and '<pubType>' and 'PPL' and 'NH' and '01/01/2022' and should get result match with legacy DB search result
    Examples:
      | pubCategory                  | pubType                        |
...
#    Then User select 'PPL' and 'NH' effective date '01/01/2022' for all material types