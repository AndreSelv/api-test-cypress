Feature: Mobile-Homeowners - MHO

#  Scenario: Delete report folder before all tests
#    Given Delete "./reports/PIM" folder

  Scenario Outline: The user validates the search result for Product line - Mobile-Homeowners, state - Oregon, Publication Category - '<pubCategory>' and Publication Type - '<pubType>' with Data base connectivity
    Then The user call search endpoint with '<pubCategory>' and '<pubType>' and 'MHO' and 'OR' and '01/01/2022' and should get result match with legacy DB search result'./cypress/data/Product_Spreadsheets/MHO.xlsx'
    Examples:
      | pubCategory                  | pubType                         |
...
#      | Manual Materials             | Download For Microsoft Office                   |
      | Manual Materials             | Multi & Sta te-specific Manuals |
...
#  Scenario: The user validates the search result for all material types
#    Then User select lobs - 'MHO', state - 'OR' effective date - '01/01/2022' for all material types